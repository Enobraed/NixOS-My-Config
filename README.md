...
Моя конфигурация NixOS является модульной, где каждый модуль (файл настройки) функционирует независимо от остальных. Это означает, что можно добавлять любой модуль (конфиг) в свои настройки.
...
My NixOS configuration is modular, where each module (customization file) functions independently of the others. This means that you can add any module (config) to your customizations.

I'm currently in the process of re-architecting all the configs and a lot of customizations have not yet been added to GitHub. They will be added and improved over time.

For each setting, I am making a small explanation in Russian for ease of use. Each parameter is thoroughly tested on my devices in different scenarios and conditions. You can add them to your config, but I am not responsible for any errors in OS operation after that.
